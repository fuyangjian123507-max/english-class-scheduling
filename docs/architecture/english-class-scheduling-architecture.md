...
MVP 阶段定义为小范围真实试运行，规模为 50 名用户、5～8 个班级，预计同时在线约 10 人。该阶段优先验证老师展示、预约申请、课表维护、预约审核、课时扣减和请假审核等核心流程，暂不追求高并发和复杂运维。
...
正式运行阶段不建议继续使用 SQLite。正式运行定义为面向完整用户群开放，或预计活跃用户明显超过试运行的 50 人规模、同时在线超过预计的 10 人，或已经出现预约高峰期排队、数据库锁等待、接口响应变慢等现象。正式上线前应将数据库迁移到 MySQL；如果运维预算允许，优先使用阿里云 RDS MySQL 承担数据库服务。
